...
If you want to keep `staging` up to date with `main` regularly:
...
# or if sure you want to overwrite staging with main: 
git reset --hard main 
git push -f
```

Use `reset --hard` with caution — it *replaces* `staging` with `main`'s content.
